...
void	Bureaucrat::executeForm(const AForm &form)
{
	...
	{
		form.execute(*this);
		std::cout << Bureaucrat::STATE << this->name_ << " executed " << form.getName() << Bureaucrat::RESET << std::endl;
	}
	catch (std::exception &e)
	{
		std::cerr << Bureaucrat::ALERT << this->name_ << " couldn’t execute " << form.getName()
				  << " because " << e.what() << "." << Bureaucrat::RESET << std::endl;
	}
}
